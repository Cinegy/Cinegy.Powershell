...
# To use - fill in the variables, manually load in (or schedule) the Football_02 scene, and set the script running.
...
# https://github.com/Cinegy/Cinegy.Powershell/blob/master/Titler/Football/Match_Squads.ps1
...
$myTeam = $match.Match.homeTeam # set to $match.Match.awayTeam if wanted
...
$xmlSetValueElem = $xmlRootElem.AppendChild($xmlDoc.CreateElement('SetValue'));
$xmlSetValueElem.SetAttribute("Name", "COACH_NAME.Text");
$xmlSetValueElem.SetAttribute("Type",'Text');
$xmlSetValueElem.SetAttribute("Value", $myTeam.coachName + " [b]" + $myTeam.coachSurname + "[/b]");

# Sort the items and store in $orderedItems
$orderedItems = $myTeam.squad.ChildNodes | Sort-Object ID

$playerNo = 1
ForEach ($player In $orderedItems) { 
    if ($playerNo -lt 10){
    ...
    $xmlSetValueElem = $xmlRootElem.AppendChild($xmlDoc.CreateElement('SetValue'));
    $xmlSetValueElem.SetAttribute("Name", "NO_" + $playerNoStr + ".Text");
    $xmlSetValueElem.SetAttribute("Type",'File');
    $xmlSetValueElem.SetAttribute("Value", $player.no);

    $xmlSetValueElem = $xmlRootElem.AppendChild($xmlDoc.CreateElement('SetValue'));
    $xmlSetValueElem.SetAttribute("Name", "NAME_" + $playerNoStr + ".Text");
    $xmlSetValueElem.SetAttribute("Type",'File');
    $xmlSetValueElem.SetAttribute("Value", $player.name + " [b]" + $player.surname + "[/b]");
