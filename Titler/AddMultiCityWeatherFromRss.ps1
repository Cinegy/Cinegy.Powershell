...
$iconUncPath = "C:\Data\dev\Git\Cinegy.Powershell\Titler\TitlerScenes\weather_icon\" # a UNC path pointing to the location of weather icons, in a location the Air server can see / access
$citiesXmlFilePath = "C:\Data\dev\Git\Cinegy.Powershell\Titler\DataSamples\CityList_update.xml" # this file should contain the list of cities you want to read the data for
$outputLogFilePath = "C:\Data\dev\Git\Cinegy.Powershell\Titler\DataSamples\PostXML-Log.xml" # Log File Path for output
...
$cityNo = 1 # key for sending multiple city data, increments in the loop
        ...
        if ($serverResponse) {
            if ($serverResponse.root.current) {
                ...
                if ($feed.root.current.temp_c) {
                    ...
                    $xmlSetValueElem.SetAttribute("Name", 'City' + $cityNo);
                    ...
                    $xmlSetValueElem = $xmlRootElem.AppendChild($xmlDoc.CreateElement('SetValue'));
                    $xmlSetValueElem.SetAttribute("Name", 'Icon' + $cityNo + '_url');
                    $xmlSetValueElem.SetAttribute("Type", 'Text');
                    $xmlSetValueElem.SetAttribute("Value", $iconUncPath + $PathEle[$PathEle.count - 2] + '\' + $PathEle[$PathEle.count - 1].ToString());	
				
                    Out-File -filepath $outputLogFilePath -InputObject  $xmlDoc.OuterXml
